...
const sanitizeHeaders = (headers) => Object.entries({ ...headers }).reduce((acc, [key, value]) => {
  if (typeof key === "string" && !key.startsWith(":")) {
    acc[key] = value;
  }
  return acc;
}, {});
const createNodeMiddleware = (serverOrigin = `http://localhost`) => (...handlers) => {
      ...
      await handleRequest(
        ...
        emitter,
        {
          ...
          async onMockedResponse(mockedResponse) {
            ...
            let returnBody;
            if (body)
              returnBody = await transformReadableStramToUint8Array(body);
...
const getNodeIntegration = (handlers) => {
  return {
...
const defaultVitePluginMswOptions = { mode: "browser", handlers: void 0, build: false };
const vitePluginMsw = ({ mode, handlers, build } = defaultVitePluginMswOptions) => ({
  ...mode === "node" && getNodeIntegration(handlers),
  ...mode === "browser" && browserIntegration({ build })
});

export { vitePluginMsw as default };
